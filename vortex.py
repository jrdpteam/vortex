...
def colorful_help():
    print(Fore.YELLOW + "vortex v3.2    " + Fore.MAGENTA + "by JRDP Team 2024")
    print(Fore.CYAN + "\nUsage:")
...
def encrypt_file(file_path, password):
    ...
    original_name_length = len(original_name_bytes).to_bytes(4, 'big')
    
    encrypted_file_path = file_path + '.vrtx'
    try:
        with open(encrypted_file_path, 'wb') as f:
            f.write(salt)
            f.write(iv)
            f.write(original_name_length)
    ...
    print(Fore.GREEN + f'File encrypted and saved as {encrypted_file_path}')
...
def decrypt_file(file_path, password):
    ...
    try:
        with open(file_path, 'rb') as f:
            salt = f.read(16)
            iv = f.read(16)
            original_name_length = int.from_bytes(f.read(4), 'big')
            original_name = f.read(original_name_length).decode('utf-8')
    ...
    print(Fore.GREEN + f'File decrypted and saved as {original_name}')
...
def main():
    parser = argparse.ArgumentParser(description=Fore.YELLOW + 'vortex v3.2    ' + Fore.MAGENTA + 'by JRDP Team 2019-2024\n' +
